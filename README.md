...
`tools/list` at `https://gateway.pipeworx.io/seo-competitors/mcp` returns the tools in the table
...
Our seo-competitors key is reserved for paid accounts, so an anonymous call to `POST https://gateway.pipeworx.io/v1/tools/seo_domain_ranked_keywords` needs your own key passed as `_apiKey` alongside the arguments. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/seo_domain_ranked_keywords`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
